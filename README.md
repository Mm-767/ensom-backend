...
늦지 않게, 서두르지 않게. 다음 일정까지 언제부터 준비하면 되는지 알려주는 앱의 백엔드.

`Spring Boot 4.1 · Java 21 · PostgreSQL (Flyway)`

프런트엔드는 별도 저장소([mju-Yarr/FE](https://github.com/mju-Yarr/FE))에 있다.
...
API 계약은 `doc/API.md`에 있다. 손으로 쓰지 않고 컨트롤러·DTO 소스에서
생성한다.
...
```bash
# 로컬엔 SMTP가 없으므로 이메일 인증을 끈다
./gradlew bootRun --args='--server.port=8090 --app.email-verification.enabled=false'
```

프런트에서 `API_BASE_URL=http://localhost:8090`으로 빌드하면 붙는다. CORS는
`http://localhost:*`를 허용하므로 포트는 자유롭게 골라도 된다.

## 비밀값

`.env`는 `.gitignore` 대상이다. 커밋하지 않는다. DB 비밀번호·JWT 시크릿·SMTP
